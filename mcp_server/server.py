...
async def bridge_set_master(
    ...
    Creates a new bridge session and saves it locally so you don't need to
    pass bridge_id or session_id to any subsequent bridge tools.

    Args:
        slug: Short human-readable name friends use to join (e.g. "frontend").
        title: Display name shown in the web UI.
        directory: Working directory of this agent (leave blank to use $PWD).
        node_url: Externally reachable HTTP URL — only needed for cross-machine setups.
    ...
    return (
        f"Bridge created — session ID saved to {sess._ID_FILE}.\n"
        f"bridgeID: {data['bridgeID']}\n"
        f"slug: {data.get('masterSlug') or '(none)'}\n"
        f"nodes: {len(nodes)}/{data['limit']}\n"
        f"Share the bridgeID or slug so friends can join with bridge_set_friend."
    )
...
async def bridge_set_friend(
    ...
    Saves the session locally so you don't need to pass bridge_id or
    session_id to subsequent bridge tools.

    Args:
        master_id_or_slug: The master's bridgeID or slug to join.
        slug: Short name for this friend node.
        title: Display name shown in the web UI.
        directory: Working directory of this friend agent (leave blank for $PWD).
        node_url: Externally reachable HTTP URL — only needed for cross-machine.
    ...
    nodes = data.get("nodes", [])
    return (
        f"Joined bridge — session ID saved to {sess._ID_FILE}.\n"
        f"bridgeID: {data['bridgeID']}\n"
        f"nodes: {', '.join(n.get('slug') or n['nodeID'] for n in nodes)}"
    )


@mcp.tool()
async def bridge_leave() -> str:
    """Leave the current bridge and clear the local session.
...
async def bridge_heartbeat() -> str:
    ...
    """
    bridge_id, session_id = sess.resolve()
...
async def bridge_push_task(
    ...
    Args:
        to_node_id: The target friend's nodeID or session ID.
                    Use bridge_get_info to see available nodes.
...
async def bridge_share_context(
    ...
    Args:
        ...
        content: The content to share. For task results prefix with the
                 task_id: "task_id: <id>\\n<result>".
        directory: Override directory (defaults to the registered node directory).
    """
    bridge_id, session_id = sess.resolve()
    role = await sess.role()
